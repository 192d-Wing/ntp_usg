...
async fn test_async_request_nist() {
    match ntp::async_ntp::request_with_timeout("time.nist.gov:123", Duration::from_secs(10)).await {
        Ok(_) => {}
        Err(e)
...
async fn test_async_request_nist_alt() {
    match ntp::async_ntp::request_with_timeout("time-a-g.nist.gov:123", Duration::from_secs(10))
        .await
    {
        Ok(_) => {}
        Err(e)
