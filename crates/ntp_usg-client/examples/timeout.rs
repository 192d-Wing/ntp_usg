...
fn main() {
    ...
    match ntp_client::request_with_timeout("time.nist.gov:123", Duration::from_secs(10)) {
        Ok(response) => {
    ...
    match ntp_client::request_with_timeout("time-a-g.nist.gov:123", Duration::from_secs(2)) {
        Ok(response) => {
    ...
    match ntp_client::request("time-b-g.nist.gov:123") {
        Ok(response) => {
    ...
    match ntp_client::request_with_timeout("time.nist.gov:123", Duration::from_millis(100)) {
